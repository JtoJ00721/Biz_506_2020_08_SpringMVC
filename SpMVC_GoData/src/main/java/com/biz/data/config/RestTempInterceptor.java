...
/*
 * RestTemplate 을 사용하여 OpenAPI 데이터를 가져올때
 * HttpHeadder에 APLICATION_JSON, APLICATION_XML 형식을
 * 지정했음에도 불구하고
 * OpenAPI 데이터를 제공하는 곳에서
 * application/json application/xml 형식으로 보내야 하는데
 * JSON, XML이 아닌 text/html, text/json, text/xml 형식으로 데이터를 보내는 경우
 * VO클래스에 데이터를 mapping하지 못해서 오류가 발생한다.
 * 
 * RestTemplate에 interceptor를 설정하여
 * 강제로 데이터를 변환시키도록 하기 위한 클래스 선언
 * ClientHttpRequestInterceptor 상속받아
 * method를 구현해주어야 한다.
 */
