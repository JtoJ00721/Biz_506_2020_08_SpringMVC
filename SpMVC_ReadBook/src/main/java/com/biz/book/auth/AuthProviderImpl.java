 ...
 * 3. authenticastion 객체에서 principal(username), cridential(password) 값을 getter하여
 * 		username과 password를 추출한다.
 * 
 * 4. UserDetailService 인터페이스를 구현한 UserDetailServiceImpl 객체를 호출하여
 * 		UserDetails 정보를 요청한다.
 * 
 * 		UserDetailsServiceImpl은 username에 해당하는 사용자 정보를
 * 		user(member) table에서 slelect하여 return 한다
 * 		이때 username에 해당하는 사용자 정보가 없으면
 * 		UseraNameNotFoundExeption을 발생시켜 spring security에게
 * 		사용자 정보가 없음을 통보한다.
 * 
 * 6. UserDetailServiceImpl로 부터 return받은 UserDetails에 담겨있는
...
public class AuthProviderImpl implements AuthenticationProvider {
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		// 3. 유효한 사용자 정보인가
		if (!userVO.isEnabled()) {
			throw new BadCredentialsException(username + "사용자 정보 사용 불가 ><");
		}

		// 4. (임시) 사용자의 숸한 리스트르 생성
