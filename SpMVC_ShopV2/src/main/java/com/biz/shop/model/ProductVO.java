...
public class ProductVO {
	...
	// 사용하는 type 이 type 0 ~ 255, -128 ~ 127까지의 값만을 저장하는 데이터
	// 메모리 공간 크기가 1byte(8bit)
	// int 형은 wrapper 클래스이다 보니 실제적으로 상당히 큰
	// 메모리 공간을 차지한다.
	// 단순히 flag와 같은 데이터를 취급할때는 char
	
	/*
	 * DB p_not_use 칼럼에 값이 NULL(IS NULL)이면 정상적인 데이터 이고
	 * NULL 이 아니면 (IS NOT NULL)이면 삭제된 데이터로 취급하기로 하였다
	 * update를 수행할때 VO의 칼럼에 값이 null이면 
	 * 		jdbcType.VARCHAR, config에 수행 설정 때문에
	 * DB에 업데이트 할떄 NULL이 아닌 다른 값이 저장되어 버린다.
	 * Dao.Update() 수행하기 전에 VO에 해당 칼럼의 값을 강제로 NULL로 해줄 필요가 있다.
	 * 하지만 기본형인 변수는 NULL을 저장하여 DB로 보낼수 없다.
	 * 
	 * 떄문에 
	 * 이 칼럼에 탕비을 byte형이 아닌 Byte wrapper 클래스형으로 설정한다.
	 */
