...
public class MemeberServiceImplV1 implements MemberService{
	 ...
	 * * 회원가입 정책(Policy)
	 * 1. tbl_member table에 아무런 데이터도 없이 초최로 가입(INSERT)된
	...
	public int insert(MemberVO memVO) {
		 ...
		 * 		 아무런 문제가 없는 코드이다
		 * 
		 * 하지만 실제상황에스는 JDBC를 통해서 서버에 원격연결을 하는 상황에서는 
		 * Count값이 실제로는 0이지만 0이아닌 -1 등의 값이 될수도 있다.ㅣ
		 * 데이터가 없을때, 0 이거나 -1인 경우가 된다면
		 * if(count == 0 || count == -1) 또는
		 * if(count <= 0)과 같은 비교연산식이 만들어 져야 한다.
		 * 
		 * 비교연산에서 가장 권장하는(좋은) 코드는
		 * 가급적 비교대상을 부등호로 한번만 비교하는 것이다.
		 * else가 잇는 상황이므로
		 * 더 값을 근접하게 찾는 연산식으로 변경하는 것이 좋다.
		 * 
		 * id(count > 0) 이 이상적인 코딩
		 */
	...
	/*
	 * 로그인 절차
	 * 1. loginVO에 담긴 정보에서 userid를 추출하여 사용자 정보를 사져오기
	 * 		findByID()
