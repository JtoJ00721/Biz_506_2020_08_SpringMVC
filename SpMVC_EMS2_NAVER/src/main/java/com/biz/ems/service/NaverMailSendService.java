...
public class NaverMailSendService {
	...
	public void sendMail(EmsVO emsVO) {
		 ...
		 * Hyper Text Markup tag로 만들어진 문서를 HTML DOC type이라고 하며 이 문서는 Web Browser를 통해 내용을
		 * 확인할 수 있다. 이처럼 Mime type을 지정해둔 문서는 인터넷으로부터 받았을때 즉시 내용을 확인할수 있다.
		 * 
		 * xMail 을 통해서 지금만드는 Mime type은 email viewer에서 확인할 수 있는 문서 형식이다. 이 문서는 메모장이나,
		 * Web Browser로 보면 그 내용을 정확히 확인하기 어렵다. 대표적으로 xmail Mime type 문서를 볼수 있는 어플리케이션에는
		...
		try {
			// 두번째값을 true : HTML 적용을 한메시지를 보낼수 있다.
			// 두번쨰값을 true : 메시지에 이미지 파일을 적용하여 보낼수 있다.
