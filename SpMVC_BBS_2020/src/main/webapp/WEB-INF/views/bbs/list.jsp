...
<script>
	document.addEventListener("DOMContentLoaded", function() {
		...
		통합으로 관리를 할수 있다.
		복잡한 event 코드같으면 td에서 처리하는 것이 효과적일수도 있지만
		...
		document.querySelector("table").addEventListener(
				...
				function(event) {
					...
					if (tag_name === "TD") {
						// 제목이 저장된 td tag에서 seq값 추출하기
						// let seq = event.target.dataset.seq

						// td tag가 클릭 되었으면 현재 클릭된 td tag 와 가장 인접한 tr tag를 참조하겠다.
						// 클릭된 td를 기준으로 tr tag에서 seq값 추출
						let seq = event.target.closest("TR").dataset.seq
						if (seq) {
							//alert(seq)
							document.location.href = "${rootPath}/bbs/detail/"
									+ seq
						}
					}
