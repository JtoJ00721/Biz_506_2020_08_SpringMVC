...
public class BBsController {
	 ...
	 * 2. 만약 해당되는 항목이 있으면 layout을 rendering할 것이다.
	 * 3. 작성된 titles-layout.xml에는 bbs/*로 설정된 항목이 있으므로
	 * 4. * 대신 list 문자열을 치환하여 마치 bbs.list 항목이 있는 것처럼 변환이 된다
	 * 5. * 대신 치환된 list 문자열은 {1}.jsp 항목에서 {1} 대신 list 문자열로 치환된다
	 * 6. 결국 bbs.list 라고 return된 문자열은 list.jsp 파일을 읽어서
