 ...
 * DB접속용 username과 password를 암호화 하여
 * 문자열을 추출
 */
public class SecurityTest {
	...
	public static void main(String[] args) {
		...
		if (saltPassword == null) {
			System.out.println("BIZ.NET 환경변수 설정값이 없지롱 ><");
			return;
		}
		...
		/*
		 * slat 값을 일반 문자열로 사용하게 되면 소스코드에 문자열이 노출되어 각종 값들을 암호화 하는 용도로 사용하는데 문제가 발생할 수 있다.
		 * 
		 * 문자열을 바로 사용하지 않고 시스템(컴퓨터운영체제)에 환경변수를 저장해 두고 환경변수를 가져다 salt 비밀번호로 사용을 한다.
		 * 
		 * BIZ.COM이라는 문자열이 노출되어도 네트워크를 통해서 salt암호를 추출하기가 매우 어려워진다.
		 * 
